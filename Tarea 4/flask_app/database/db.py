...
# Inicio Agregados Tarea 4.

# Aquí sacamos de la base de datos solamente los atributos que queremos analizar para la búsqueda,
# es decir, [actividad.nombre, actividad.dia, actividad.tipo, miembro.nombre, comuna.nombre], en
# donde se debe tener que (miembro.id == actividad.miembro_id) y (comuna.id = miembro.comuna_id).
def actividad_miembro_comuna_detalle(search_term=None):
    session = SessionLocal()
    try:
        query = (
            session.query(
                Actividad.id.label("actividad_id"),
                Actividad.nombre.label("nombreActividad"),
                Actividad.dia,
                Actividad.tipo,
                Miembro.id.label("miembro_id"),
                Miembro.nombre.label("nombreMiembro"),
                Comuna.nombre.label("nombreComuna"),
            )
            .join(Miembro, Actividad.miembro_id == Miembro.id)
            .join(Comuna, Miembro.comuna_id == Comuna.id)
        )
        ...
        if search_term:
            patron = f"%{search_term.strip().lower()}%"
            query = query.filter(
                func.lower(Actividad.nombre).like(patron)
                | func.lower(Actividad.tipo).like(patron)
                | func.lower(Miembro.nombre).like(patron)
                | func.lower(Comuna.nombre).like(patron)
            )
        ...
        return [
            {
                ...
                "nombreMiembro": fila.nombreMiembro,
